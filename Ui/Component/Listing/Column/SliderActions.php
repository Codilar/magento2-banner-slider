        ...
        if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                ...
                if (isset($item[self::ENTITY_ID])) {
                    $item[$name]['edit'] = [
                        'href' => $this->url->getUrl('codbansli/slider/edit', [self::ENTITY_ID => $item[self::ENTITY_ID]]),
                        'label' => __('Edit'),
                        '__disableTmpl' => true,
                    ];
                    $title = $this->escaper->escapeHtml($item['title']);
                    $item[$name]['delete'] = [
                        'href' => $this->url->getUrl('codbansli/slider/delete', [self::ENTITY_ID => $item[self::ENTITY_ID]]),
                        'label' => __('Delete'),
                        'confirm' => [
